...
import { createInvoicesService, deleteInvoiceService, getInvoicesByUserService, getInvoicesService, getClientsByPeriodService, updatePaymentService } from "../services/invoice.service.js";
...
export const getInvoicesByUserController = async (req, res) => {
  try {
    ...
    return res.status(200).json({ success: true, response: invoicesByUser });
  } catch (error) {
    res.json({ success: false, error: error.message });
  }
};
...
export const updatePaymentController = async (req, res) => {
  try {
    const { id } = req.user;
    const { _id } = req.query
    if (!mongoose.Types.ObjectId.isValid(id)) throw new Error('El ID del usuario no tiene el formato válido');
    if (!mongoose.Types.ObjectId.isValid(_id)) throw new Error('El ID de la factura no tiene el formato válido');
    const invoice = await updatePaymentService({id, _id});
    return res.status(200).send({ success: true, response: invoice });
  } catch (error) {
    res.json({ success: false, error: error.message });
  }
};